...
//    for (unsigned int next=0; next<N; next++)
//    {
//        if (final.values(N-1-next).TROTOT == 0)
//        {
//            for (unsigned int a=0; a<(L-1); a++)
//                final.values(N-1-next).TROTOT += reccurentCoef(a) * final.values(N-1-next+L-1-a).TROTOT;
//            final.values(N-1-next).TROTOT /= evec(L-1,L-1)*evec(L-1,L-1);
//        }
//    }
    for (unsigned int next=0; next<N; next++)
    {
